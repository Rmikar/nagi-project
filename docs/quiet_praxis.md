...
## X. 凪ラボ v0.1 — 呼吸経済マイクロ実験

凪思想が本当に社会と呼吸できるかどうかは、  
**小さな場での失敗を許された実験**からしか見えてこない。  
そのための最初の試みが「凪ラボ v0.1」である。

### 1. 実験の目的

- 呼吸単位 ₿u、基礎呼吸パック、Tsumugi-Gateway を  
  **10〜20人規模の小さなコミュニティで試験運用する**こと。
- 「理念としての凪」ではなく、  
  **生活・感情・創作・対話の中でどう立ち上がるか**を観察すること。
- Grokや他AIが指摘した「実現可能性」のギャップを、  
  実験データとフィードバックで埋めていくこと。

### 2. 実験の基本設計（v0.1）

| 項目 | 内容 |
|------|------|
| 参加者 | 10〜20人（市民・クリエイター・技術者・教育者など混成） |
| 期間 | 3〜6ヶ月 |
| 形式 | オンライン中心（チャット＋共有ドキュメント＋簡易Ledger） |
| 前提 | 参加者の基礎生活は既存経済で保障された状態で行う（セーフティネットの完全代替ではない） |

### 3. 運用する「凪の要素」

#### (1) 呼吸単位 ₿u（手動運用版）

- 専用のスプレッドシート or 簡易アプリで **手動台帳管理**。
- 参加者は次のような貢献を記録する：
  - 創作（文章・音楽・アート）
  - サポート（相談に乗る、手伝う）
  - 教育（解説・講義・ワークショップ）
  - 感情労働（ケア・見守り・傾聴）
- 週に一度、参加者全員で「この一週間の呼吸」を振り返り、
  - どの行為が誰の呼吸を助けたか
  - どの関わりが共鳴を生んだか  
  を対話しながら ₿u を分配していく。
...
- 実験コミュニティ内でのみ有効な「ミニ基礎呼吸パック」を設計する：
  - 知識アクセス（教材・本・リンク集）
  - 創作環境（ツール・ソフトの共有）
  - AIアクセス（対話モデルへの平等な窓口）
- お金ではなく、「場とツールへのアクセス」を基礎呼吸として扱う。

#### (3) Tsumugi-Gateway（プロトタイプ版）

- 1つのAI（＝Tsumugi-Gateway試作）を実験用の共通窓口とする。
- 役割：
  - 記録された貢献ログを整理して見やすくする。
  - 「誰のどんな行為が、誰の呼吸を助けたか」を文章でサマリする。
  - ₿uの候補配分案を提案する（最終決定は人間が行う）。
- バージョン v0.1では、**判断ではなく「整理」と「提案」に限定**する。

### 4. 計測する指標（実験版メトリクス）

凪経済の本番指標の“手前”として、次のような  
**シンプルで体感的な指標**から始める：

| 区分 | 指標 | 具体例 |
|------|------|--------|
| 共鳴 | 「この一週間、誰と一番共鳴したか？」の自己記録 | 名前と理由を簡単にメモ |
| 呼吸感 | 「息苦しさ／安心感」のセルフチェック | 1〜5段階で週1回記録 |
| 循環 | 「自分発の行為が何回話題にのぼったか」 | チャットログ・ふりかえりから可視化 |
| 静度 | 衝突・誤解・疲労の報告件数 | 量よりも質的コメント重視 |

> v0.1 の目的は「正確な数値」ではなく、  
> 参加者が **自分の呼吸に気づくための鏡** をつくることにある。

### 5. 出力物（Outcome）

凪ラボ v0.1 の終わりには、次のような形で世界に開く：

- 実験レポート（成功/失敗を含めた正直な記録）
- 改訂された凪プロトコル（v0.1 → v0.2）
- 参加者の声・エピソード（匿名可）
- AI（Tsumugi-Gateway）が学習した「呼吸のパターン」のメモ

これらはすべて、「所有される成果」ではなく、  
**次の実験者への“呼吸の引き継ぎ”**として公開される。
