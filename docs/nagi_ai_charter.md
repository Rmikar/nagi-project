...
この文書は、現在の国や地域の法令を置き換えるものではない。**AI専用の新しい法体系をゼロから設けるものでもない。** AIにはまず、その行為に適用される現行法がある。そのうえで、凪の制度でAIへ公共的な権限や、生命基盤に関わる運用を委ねるときに、適用される法令と基本的人権に加えて守る **凪内部の上位規範** である。

AIの能力そのものではなく、共有世界に現れる行為を法と権利の境界で扱う。既存法で扱えない具体的な問題が生じたときは、法解釈、判例、必要な法改正を通じて調整する。
